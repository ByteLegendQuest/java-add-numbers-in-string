...
`git clone https://github.com/ByteLegendQuest/java-add-numbers-in-string`

A project directory named `java-add-numbers-in-string` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-add-numbers-in-string` directory which is created just now.
